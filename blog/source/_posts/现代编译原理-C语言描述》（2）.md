...
+ 可选（alternation）：对于给定的两个正则表达式$M$和$N$，可选操作符（|）形成一个新的正则表达式$M|N$。如果一个字符串属于语言$M$或$N$，则它属于语言$M|N$。因此$a，b$组成的语言包含$a$和$b$这两个字符串。
...
这次的程序分析题比第一次友好很多啊,只需要修改一个文件`tiger.lex`就行了,内容也不难就是写几个正则表达式.
```lex
```
#### 推荐阅读
